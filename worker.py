...
class Worker(threading.Thread):
    ...
    def __order_menu(self, final_category):
        ...
        while True:
            ...
            # If a Add to Cart button has been pressed...
                ...
                number = Decimal("0.2")
                cart[callback.message.message_id][1] += number
    ...
    def __get_cart_value(self, cart):
        ...
        for product in cart:
            number = Decimal("0.01")
            num = cart[product][0].price * number * cart[product][1]
            value += float(num)
            #cart[product][0].price * number * cart[product][1]
        return value
    ...
    def __get_cart_summary(self, cart):
        ...
        for product_id in cart:
            if cart[product_id][1] > 0:
                kGs = cart[product_id][1]
                kGs_num = Decimal(str(kGs))
    ...
    def __edit_product_menu(self, product: Optional[db.Product] = None):
        ...
        if product:
                self.bot.send_message(self.chat.id, self.loc.get("edit_current_value", value=escape(product.categories)),
                                      reply_markup=cancel)
        categories = self.__wait_for_regex(r"(.*)", cancellable=bool(product))
        print(categories)    
        ...
        # If a product is being edited...
            ...
            product.categories = categories if not isinstance(categories, CancelSignal) else product.categories.capitalize()
